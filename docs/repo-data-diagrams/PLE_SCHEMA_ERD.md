...
erDiagram
    ...
    FORM_5500 ||--o{ SCHEDULE_A : "has schedules"

    FORM_5500 {
        text ack_id PK "DOL acknowledgment ID"
        varchar ein "Employer ID (links to company)"
        ...
        text spons_dfe_mail_us_city "City"
        text spons_dfe_mail_us_state "State"
        integer tot_partcp_boy_cnt "Participant count (BOY)"
        numeric tot_assets_boy_amt "Total assets (BOY)"
        date plan_eff_date "Plan effective date"
        date form_plan_year_begin "Plan year begin"
        date form_plan_year_end "Plan year end (renewal)"
        varchar type_pension_bnft_code "Pension benefit type"
        varchar type_welfare_bnft_code "Welfare benefit type"
    }
    ...
    FORM_5500_SF {
        ...
        text plan_name "Plan name"
        integer participant_count "Participant count"
        numeric total_assets "Total assets"
        date plan_year_end "Plan year end"
    }

    SCHEDULE_A {
        text ack_id PK "DOL acknowledgment ID"
        text insurance_company_name "Insurance carrier"
        varchar prov_contract_num "Contract number"
        integer covered_persons_cnt "Covered lives"
        numeric premium_amount "Premium information"
        numeric commission_amount "Broker commission"
    }
...
| **DOL Filings** | 04.04.03 | form_5500, form_5500_sf, schedule_a |
...
4. **person_unique_id** links people to scores and movement history
