...
BRIDGE LOGIC:
  outreach.outreach.domain → company.company_master.website_url
...
def preflight_checks(cur):
    ...
    # Check 3: Count source outreach_ids via outreach.outreach (the spine)
    cur.execute("""
        SELECT COUNT(*) as total_outreach_ids
        FROM outreach.outreach
    """)
    result = cur.fetchone()
    total_source = result['total_outreach_ids']
    print(f"  ✓ Source outreach_ids in outreach.outreach: {total_source:,}")
    
    # Check 4: Count matchable outreach_ids (have domain AND company_master match)
    cur.execute("""
        SELECT COUNT(DISTINCT o.outreach_id) as matchable
        FROM outreach.outreach o
        JOIN company.company_master cm 
            ON cm.website_url ILIKE '%' || o.domain || '%'
        WHERE o.domain IS NOT NULL 
          AND o.domain != ''
    """)
    matchable = cur.fetchone()['matchable']
    print(f"  ✓ Matchable outreach_ids (domain→company_master): {matchable:,}")
    
    # Check 5: Count existing slots
    cur.execute("""
    ...
    # Check 6: Pre-counts by slot_type
    cur.execute("""
    ...
    # Check 7: Count people_master and people_candidate for baseline
    ...
    return {
        ...
        'matchable_outreach_ids': matchable,
...
def execute_bulk_seed(cur, slot_types):
    ...
    Uses domain-based lookup to get company_unique_id from company_master.
    """
    print("\n" + "=" * 70)
    ...
    for slot_type in slot_types:
        ...
        # Count existing for this slot_type
        cur.execute("""
        ...
        # Count how many could be inserted (matchable and missing)
        cur.execute("""
            SELECT COUNT(DISTINCT o.outreach_id) as to_insert
            FROM outreach.outreach o
            JOIN company.company_master cm 
                ON cm.website_url ILIKE '%%' || o.domain || '%%'
            LEFT JOIN people.company_slot cs 
                ON cs.outreach_id = o.outreach_id 
                AND cs.slot_type = %s
            WHERE o.domain IS NOT NULL 
              AND o.domain != ''
        ...
        print(f"    Missing {slot_type} slots (matchable): {to_insert:,}")
        ...
        # Execute INSERT via domain-based company_master lookup
        # Using DISTINCT ON to avoid duplicates when multiple company_master rows match
        cur.execute("""
            ...
            WHERE o.domain IS NOT NULL 
              AND o.domain != ''
              AND cs.company_slot_unique_id IS NULL
            ORDER BY o.outreach_id, cm.company_unique_id
        """, (slot_type, slot_type))
...
def run_validation_queries(cur, preflight_data):
    ...
    # Query 6: Verify all new slots have status='open'
    cur.execute("""
        SELECT COUNT(*) as non_open
        FROM people.company_slot
        WHERE creation_reason = 'bulk_seed_structure_only'
          AND slot_status != 'open'
    """)
    non_open = cur.fetchone()['non_open']
    print(f"\n  Non-open slots from bulk seed: {non_open}")
    if non_open > 0:
        print("  ❌ ERROR: Some slots created with wrong status!")
    else:
        print("  ✓ All bulk-seeded slots have status='open'")
...
def generate_report(preflight_data, seed_results, total_inserted, total_skipped, validation):
    ...
    print(f"║    Matchable (domain→company): {preflight_data['matchable_outreach_ids']:>33,} ║")
    ...
    status_ok = "✓ ALL OPEN" if validation['non_open_count'] == 0 else "❌ ERROR"
    ...
    print(f"║    slot_status validation: {status_ok:>37} ║")
    print("╠" + "═" * 68 + "╣")
...
def main():
    ...
    print("  Target: ALL outreach_ids from outreach.outreach")
    print("  Bridge: domain → company_master.website_url")
    ...
    try:
        ...
        if (validation['people_master_delta'] != 0 or 
            ...
            validation['kill_switch_enabled'] or
            validation['non_open_count'] > 0):
