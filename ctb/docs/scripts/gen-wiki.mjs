...
function writeBranchWiki(branch) {
  ...
  const contractsSection = branch.contracts?.map(c => `- \`${c}\``).join('\n') || '- No contracts specified';
  const metricsSection = branch.metrics?.map(m => `- ${m}`).join('\n') || '- No metrics specified';
  const dashboardsSection = branch.dashboards?.map(d => `- [${d}](${d})`).join('\n') || '- No dashboards specified';
  const risksSection = branch.risks?.map(r => `- ⚠️ ${r}`).join('\n') || '- No risks identified';
...
function generateOverview() {
  ...
  const altitudeGroups = branches.reduce((acc, b) => {
...
${Object.entries(altitudeGroups).map(([alt, branchList]) => 
  `### ${alt} Level\n${branchList.map(b => `- ${b.title}`).join('\n')}`
).join('\n\n')}
