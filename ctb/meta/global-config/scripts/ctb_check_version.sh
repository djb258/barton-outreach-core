...
IMO_REPO="https://github.com/djb258/imo-creator.git"
log_info "Checking remote version from: $IMO_REPO"

if git clone --depth 1 --quiet "$IMO_REPO" "$TMP_DIR/imo-creator" 2>/dev/null; then
  REMOTE_VER=$(get_version_from_json "$TMP_DIR/imo-creator/global-config/ctb_version.json")
