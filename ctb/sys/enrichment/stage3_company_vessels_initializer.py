...
def run_pipeline():
    ...
    print("=" * 70)
    print("STAGE 3 COMPANY VESSELS INITIALIZER")
    print("=" * 70)
    ...
    log = {
        ...
        'number_with_patterns': 0,
        'number_with_phones': 0,
        'errors': []
    }
    ...
    print(f"  Loaded enriched_matches: {len(enriched_df):,}")
    print(f"  Loaded companies: {len(companies_df):,}")

    # Merge to ensure we have all fields
    ...
    # Use website_url if url is missing
    if 'url' not in merged_df.columns:
        merged_df['url'] = merged_df['website_url']
    else:
        merged_df['url'] = merged_df['url'].fillna(merged_df['website_url'])
    ...
    merged_df['domain'] = merged_df['url'].apply(get_domain)

    # Filter by confidence
    print("\n[2] Filtering by confidence...")

    # Handle different column names for confidence
    conf_col = 'confidence' if 'confidence' in merged_df.columns else 'confidence_label'
    if conf_col not in merged_df.columns:
        # Create confidence from final_score
        merged_df['confidence'] = merged_df['final_score'].apply(
            lambda x: 'HIGH_CONFIDENCE' if x >= 0.75 else ('MEDIUM_CONFIDENCE' if x >= 0.60 else 'LOW_CONFIDENCE')
        )
        conf_col = 'confidence'

    high_df = merged_df[merged_df[conf_col] == 'HIGH_CONFIDENCE']
    medium_df = merged_df[merged_df[conf_col] == 'MEDIUM_CONFIDENCE']
    low_df = merged_df[merged_df[conf_col] == 'LOW_CONFIDENCE']
    ...
    print(f"  HIGH_CONFIDENCE: {len(high_df):,}")
    print(f"  MEDIUM_CONFIDENCE: {len(medium_df):,}")
    print(f"  LOW_CONFIDENCE (skipped): {len(low_df):,}")

    # Process only HIGH + MEDIUM
    process_df = pd.concat([high_df, medium_df], ignore_index=True)
    print(f"\n  Processing: {len(process_df):,} companies")
    ...
    for batch_num in range(total_batches):
        ...
        for row in batch:
            try:
                ...
                if result['company_email_pattern'] != 'unknown':
                    batch_patterns += 1
                    log['number_with_patterns'] += 1

                if result['company_phone_main'] or result['company_phone_hr']:
                    batch_phones += 1
                    log['number_with_phones'] += 1
        ...
        print(f"    Patterns found: {batch_patterns}, Phones found: {batch_phones}")
    ...
    print(f"  Saved: {csv_file} ({len(results_df):,} records)")
    ...
    print(f"  Saved: {jsonl_file} ({len(all_results):,} records)")
    ...
    print("=" * 70)
    print(f"Total companies processed: {log['total_companies_processed']:,}")
    print(f"HIGH_CONFIDENCE: {log['number_high_confidence']:,}")
    print(f"MEDIUM_CONFIDENCE: {log['number_medium_confidence']:,}")
    print(f"LOW_CONFIDENCE skipped: {log['number_low_confidence_skipped']:,}")
    print(f"Companies with email patterns: {log['number_with_patterns']:,}")
    print(f"Companies with phones: {log['number_with_phones']:,}")
