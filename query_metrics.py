...
print('8. SLOT FILL RATES')
print('-' * 80)
cur.execute("SELECT COUNT(DISTINCT o.outreach_id), COUNT(DISTINCT CASE WHEN cs_ceo.id IS NOT NULL THEN o.outreach_id END), COUNT(DISTINCT CASE WHEN cs_cfo.id IS NOT NULL THEN o.outreach_id END), COUNT(DISTINCT CASE WHEN cs_hr.id IS NOT NULL THEN o.outreach_id END) FROM outreach.outreach o LEFT JOIN people.company_slot cs_ceo ON o.outreach_id = cs_ceo.outreach_id AND cs_ceo.slot_type = 'CEO' LEFT JOIN people.company_slot cs_cfo ON o.outreach_id = cs_cfo.outreach_id AND cs_cfo.slot_type = 'CFO' LEFT JOIN people.company_slot cs_hr ON o.outreach_id = cs_hr.outreach_id AND cs_hr.slot_type = 'HR'")
stats = cur.fetchone()
