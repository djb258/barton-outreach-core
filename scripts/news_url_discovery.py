#!/usr/bin/env python3
"""
News/Blog URL Discovery Script

Discovers news, blog, and press URLs for companies.
Stores in company_source_urls for later content extraction.
...
# News/Blog URL patterns
URL_PATTERNS = {
    'news_page': ['/news', '/newsroom'],
    'blog_page': ['/blog', '/insights'],
    'press_page': ['/press', '/press-releases', '/media'],
}
...
def discover_company_urls(company: dict) -> dict:
    """Discover news/blog URLs for a company."""
...
def get_companies_for_news_discovery(conn, limit: int) -> List[dict]:
    """Get companies needing news URL discovery - only those with confirmed working websites."""

    with conn.cursor(cursor_factory=RealDictCursor) as cur:
        # Only check companies that already have leadership/team/about pages (confirmed working)
        cur.execute("""
            WITH has_working_site AS (
            ...
            has_news AS (
                SELECT DISTINCT company_unique_id
                FROM company.company_source_urls
                WHERE source_type IN ('news_page', 'blog_page', 'press_page')
            )
            SELECT DISTINCT
                cm.company_unique_id as doctrine_company_id,
                LOWER(REGEXP_REPLACE(REGEXP_REPLACE(cm.website_url, '^https?://', ''), '/.*$', '')) as domain
            FROM company.company_master cm
            JOIN has_working_site hw ON cm.company_unique_id = hw.company_unique_id
            WHERE cm.company_unique_id NOT IN (SELECT company_unique_id FROM has_news)
            LIMIT %s
        """, (limit,))
        return cur.fetchall()
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("NEWS/BLOG URL DISCOVERY")
    logger.info("=" * 70)
    logger.info(f"Workers: {MAX_WORKERS}, Timeout: {REQUEST_TIMEOUT}s, Batch: {BATCH_SIZE}")
    logger.info("Patterns: news, blog, press, media, insights")
    ...
    try:
        ...
        logger.info("Finding companies without news URLs...")
        companies = get_companies_for_news_discovery(conn, MAX_COMPANIES)
        conn.close()
        ...
        logger.info(f"News/Blog URLs found: {STATS['urls_found']:,}")
