/**
 * Doctrine Spec:
 * - Barton ID: 08.05.02.07.10000.005
...
export default async function handler(
  ...
  try {
    ...
    try {
      ...
      await mcpBridge.executeNeonOperation('EXECUTE_SQL', {
      sql: `UPDATE marketing.campaigns
        SET status = 'active',
            launched_at = NOW(),
            updated_at = NOW()
          ...
          if (launchResult.success) {
            ...
            if (launchResult.execution_ids) {
              for (const execId of launchResult.execution_ids) {
                await mcpBridge.executeNeonOperation('EXECUTE_SQL', {
      sql: `UPDATE marketing.campaign_executions
      ...
      await mcpBridge.executeNeonOperation('EXECUTE_SQL', {
      sql: `UPDATE marketing.campaign_audit_log
...
async function getCampaignDetails(campaignId: string): Promise<any> {
  try {
    ...
    console.error('[MCP Get Campaign Details] Error:', error);
    return null;
  }
}
...
async function prepareCampaignData(campaign: any, tool: string): Promise<any> {
  ...
  const recipients = await mcpBridge.executeNeonOperation('QUERY_ROWS', {
      sql: `SELECT
      pm.unique_id,
      pm.first_name,
      pm.last_name,
    ...
    if (!recipients.success) {
      throw new Error(`MCP operation failed: ${recipients.error}`);
    }
